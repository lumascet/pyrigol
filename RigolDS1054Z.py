	...
	def get_measurement(self, channel=1, meas_type=max_voltage):
		...
		if (meas_type.return_type == 'float'):
			...
			if (meas_type.unit == '%%'):
				...
				print ("Channel " + str(channel) + " " + meas_type.name + " value is {0} %".format(percentage_reading))
			else:
			...
			print ("Channel " + str(channel) + " " + meas_type.name + " value is {0} ".format(reading) + meas_type.unit)
		else:
