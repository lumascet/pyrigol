...
class RigolDS1054Z:
    ...
    max_voltage = measurement(name='max_voltage',          command='VMAX', unit='Volts',
                              return_type='float', description='voltage value from the highest point of the waveform to the GND')
    min_voltage = measurement(name='min_voltage',          command='VMIN', unit='Volts',
                              return_type='float', description='voltage value from the lowest point of the waveform to the GND')
    peak_to_peak_voltage = measurement(name='peak_to_peak_voltage', command='VPP',  unit='Volts',
                                       return_type='float', description='voltage value from the highest point to the lowest point of the waveform')
    top_voltage = measurement(name='top_voltage',          command='VTOP', unit='Volts',
                              return_type='float', description='voltage value from the flat top of the waveform to the GND')
    base_voltage = measurement(name='base_voltage',         command='VBAS', unit='Volts',
                               return_type='float', description='voltage value from the flat base of the waveform to the GND')
    top_to_base_voltage = measurement(name='top_to_base_voltage',  command='VAMP', unit='Volts',
                                      return_type='float', description='voltage value from the top of the waveform to the base of the waveform')
    average_voltage = measurement(name='average_voltage',      command='VAVG', unit='Volts',
                                  return_type='float', description='arithmetic average value on the whole waveform or on the gating area')
    rms_voltage = measurement(name='rms_voltage',          command='VRMS', unit='Volts',
                              return_type='float', description='root mean square value on the whole waveform or the gating area')
    upper_voltage = measurement(name='upper_voltage',        command='VUP',  unit='Volts',
                                return_type='float', description='actual voltage value corresponding to the threshold maximum value')
    mid_voltage = measurement(name='mid_voltage',          command='VMID', unit='Volts',
                              return_type='float', description='actual voltage value corresponding to the threshold middle value')
    lower_voltage = measurement(name='lower_voltage',        command='VLOW', unit='Volts',
                                return_type='float', description='actual voltage value corresponding to the threshold minimum value')
    overshoot_voltage = measurement(name='overshoot_percent',    command='OVER', unit='%%',            return_type='float',
                                    description='ratio of the difference of the maximum value and top value of the waveform to the amplitude value')
    preshoot_voltage = measurement(name='preshoot_percent',     command='PRES', unit='%%',            return_type='float',
                                   description='ratio of the difference of the minimum value and base value of the waveform to the amplitude value')
    variance_voltage = measurement(name='variance_voltage',     command='VARI', unit='Volts',         return_type='float',
                                   description='average of the sum of the squares for the difference between the amplitude value of each waveform point and the waveform average value on the whole waveform or on the gating area')
    period_rms_voltage = measurement(name='period_rms_voltage',   command='PVRMS', unit='Volts',
                                     return_type='float', description='root mean square value within a period of the waveform')
    period_time = measurement(name='period_time',          command='PER',  unit='Seconds',       return_type='float',
                              description='time between the middle threshold points of two consecutive, like-polarity edges')
    frequency = measurement(name='frequency',            command='FREQ', unit='Hz',
                            return_type='float', description='reciprocal of period')
    rise_time = measurement(name='rise_time',            command='RTIM', unit='Seconds',       return_type='string',
                            description='time for the signal amplitude to rise from the threshold lower limit to the threshold upper limit')
    fall_time = measurement(name='fall_time',            command='FTIM', unit='Seconds',       return_type='string',
                            description='time for the signal amplitude to fall from the threshold upper limit to the threshold lower limit')
    positive_width_time = measurement(name='positive_width_time',  command='PWID', unit='Seconds',       return_type='float',
                                      description='time difference between the threshold middle value of a rising edge and the threshold middle value of the next falling edge of the pulse')
    negative_width_time = measurement(name='negative_width_time',  command='NWID', unit='Seconds',       return_type='float',
                                      description='time difference between the threshold middle value of a falling edge and the threshold middle value of the next rising edge of the pulse')
    positive_duty_percent = measurement(name='positive_duty_ratio',  command='PDUT', unit='%%',
                                        return_type='float', description='ratio of the positive pulse width to the period')
    negative_duty_percent = measurement(name='negative_duty_ratio',  command='NDUT', unit='%%',
                                        return_type='float', description='ratio of the negative pulse width to the period')
    max_voltage_time = measurement(name='max_voltage_time',     command='TVMAX', unit='Seconds',
                                   return_type='float', description='time corresponding to the waveform maximum value')
    min_voltage_time = measurement(name='min_voltage_time',     command='TVMIN', unit='Seconds',
                                   return_type='float', description='time corresponding to the waveform minimum value')
    positive_pulse_number = measurement(name='positive_pulse_number', command='PPUL', unit='Occurances',    return_type='int',
                                        description='number of positive pulses that rise from below the threshold lower limit to above the threshold upper limit')
    negative_pulse_number = measurement(name='negative_pulse_number', command='NPUL', unit='Occurances',    return_type='int',
                                        description='number of negative pulses that fall from above the threshold upper limit to below the threshold lower limit')
    positive_edges_number = measurement(name='positive_edges_number', command='PEDG', unit='Occurances',    return_type='int',
                                        description='number of rising edges that rise from below the threshold lower limit to above the threshold upper limit')
    negative_edges_number = measurement(name='negative_edges_number', command='NEDG', unit='Occurances',    return_type='int',
                                        description='number of falling edges that fall from above the threshold upper limit to below the threshold lower limit')
    rising_delay_time = measurement(name='rising_delay_time',    command='RDEL', unit='Seconds',       return_type='string',
                                    description='time difference between the falling edges of source 1 and source 2. Negative delay indicates that the selected falling edge of source 1 occurred after that of source 2')
    falling_delay_time = measurement(name='falling_delay_time',   command='FDEL', unit='Seconds',       return_type='string',
                                     description='time difference between the falling edges of source 1 and source 2. Negative delay indicates that the selected falling edge of source 1 occurred after that of source 2')
    rising_phase_ratio = measurement(name='rising_phase_ratio',   command='RPH',  unit='Degrees',
                                     return_type='float', description='rising_delay_time / period_time x 360 degrees')
    falling_phase_ratio = measurement(name='falling_phase_ratio',  command='FPH',  unit='Degrees',
                                      return_type='float', description='falling_delay_time / period_time x 360 degrees')
    positive_slew_rate = measurement(name='positive_slew_rate',   command='PSLEW', unit='Volts / Second', return_type='float',
                                     description='divide the difference of the upper value and lower value on the rising edge by the corresponding time')
    negative_slew_rate = measurement(name='negative_slew_rate',   command='NSLEW', unit='Volts / Second', return_type='float',
                                     description='divide the difference of the lower value and upper value on the falling edge by the corresponding time')
    waveform_area = measurement(name='waveform_area',        command='MAR',  unit='Volt Seconds',  return_type='float',
                                description='algebraic sum of the area of the whole waveform within the screen. area of the waveform above the zero reference is positive and the area of the waveform below the zero reference is negative')
    first_period_area = measurement(name='first_period_area',    command='MPAR', unit='Volt Seconds',  return_type='float',
                                    description='algebraic sum of the area of the first period of the waveform on the screen. area of the waveform above the zero reference is positive and the area of the waveform below the zero reference is negative')
# all available: {VMAX|VMIN|VPP|VTOP|VBASe|VAMP|VAVG| VRMS|OVERshoot|PREShoot|MARea|MPARea| PERiod|FREQuency|RTIMe|FTIMe|PWIDth| NWIDth|PDUTy|NDUTy|RDELay|FDELay| RPHase|FPHase|TVMAX|TVMIN|PSLEWrate| NSLEWrate|VUPper|VMID|VLOWer|VARIance| PVRMS|PPULses|NPULses|PEDGes|NEDGes}
# two required (RDELay, FDELay, RPHase, and FPHase)
    ...
    def write_scope_settings_to_file(self, filename=''):
        ...
        if (filename == ''):
            filename = "rigol_settings_" + \
                datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + ".stp"
        fid = open(filename, 'wb')
        fid.write(raw_data)
        fid.close()
